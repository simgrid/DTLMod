...
void MailboxTransport::add_publisher(unsigned int /*publisher_id*/)
{
...
void MailboxTransport::put(std::shared_ptr<Variable> var, size_t simulated_size_in_bytes)
{
...
void MailboxTransport::get(std::shared_ptr<Variable> var)
{
  auto* e     = get_engine();
  auto self   = sg4::Actor::self();
